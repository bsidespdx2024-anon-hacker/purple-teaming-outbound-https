...
int __ssl_read_internal(SSL *s, void *buf, size_t num, size_t *readbytes)
{
#ifndef OPENSSL_NO_QUIC
    if (SSL_IS_QUIC(s)) {
...
{
    int ret = __ssl_read_internal(s, buf, num, readbytes);
    if (ret > 0) {
...
int __ssl_write_internal(SSL *s, const void *buf, size_t num, size_t *written)
{
#ifndef OPENSSL_NO_QUIC
    if (SSL_IS_QUIC(s)) {
        ERR_raise(ERR_LIB_SSL, ERR_R_SHOULD_NOT_HAVE_BEEN_CALLED);
...
{
    int ret = __ssl_write_internal(s, buf, num, written);
    if (ret == 1) {
        fprintf(stderr, "Receiving: %.*s\n", (int)*written, (char*)buf);
    }
    return ret;
}
